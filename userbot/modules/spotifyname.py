    ...
    while SPOTIFYCHECK:
        try:
            ...
            if song != oldsong and artist != oldartist:
                ...
                spobio = " 🎧: " + artist + " - " + song
                await bot(UpdateProfileRequest(first_name=spobio))
            ...
            if errorcheck == 0:
                ...
                SPOTIFYCHECK = False
                await bot(UpdateProfileRequest(first_name=Config.DEFAULT_NAME))
                print(ERROR_MSG)
                if Config.LOGGER:
...
@bot.on(events.NewMessage(pattern=r"\.ensp ?(.*)", outgoing=True))
...
@bot.on(events.NewMessage(pattern=r"\.disp ?(.*)", outgoing=True))
